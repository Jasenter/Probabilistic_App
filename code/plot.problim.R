...
##  Probability limits can be filled, or lines, or bars, see arguments for further details. 
  ...
  if(!is.null(pred)) {
    if(!switch.xy) {
      if (!is.null(pred.pch)) {points(x,y=pred,pch=pred.pch,col=pred.col,bg=pred.fill.col)} else {pred.pch=NA}
      if (!is.null(pred.lty)) {lines(x,y=pred,lwd=pred.lwd,lty=pred.lty,col=pred.col)} else {pred.lty=NA}
    } else {
    ...
    leg.txt=c(pred.name,leg.txt); leg.pch=c(pred.pch,leg.pch); leg.lty=c(pred.lty,leg.lty);leg.col=c(pred.col,leg.col);pt.bg=c(pred.fill.col,pt.bg)
  }
  # Plot observed data
  if(!is.null(obs)) {
    if(!switch.xy) {
