  ...
  for (r in 1:length(resid.outlier.thres)){
   ...
   if (any(!is.null(user.par))){ # if user.par is NA, then do not set-it
     if (!is.na(user.par)) {
       ...
       #if (nplot>=5) {layout(matrix(c(1,2,3,4),2,2,byrow=T)); op=par(oma=c(0,0,4,0),mar=c(4,4,3,2)+0.1)}
       #if (nplot>=4 | nplot==3) op=par(mfrow=c(1,2),oma=c(0,0,4,0),mar=c(4,4,3,2)+0.1)
   ...
   for (p in 1:length(plot.type)) {
     ...
     #if (add.leg) legend("topright", leg=scen[s], lty=1, col=(s+1), bty='n',cex=cex.leg)
...
# Examples
attr(plot.residuals.multi,"ex") <- function(){
...
#Plot the residuals
plot.residuals.multi(residuals=residuals,pred=pred)

#Plot the residuals, remove all residuals >1.5
plot.residuals.multi(residuals=residuals,pred=pred,resid.outlier.thres=1.5)

#Only plot two diagnostics 
plot.residuals.multi(residuals=residuals,pred=pred,plot.type=c("density","pred"),resid.outlier.thres=1.5)

#Plot residuals using a mask, of values for 100-200
plot.residuals.multi(residuals=residuals,pred=pred,mask=c(rep(FALSE,100),rep(TRUE,200),rep(FALSE,700)))

#Test error Handling, when all(|residuals|)>resid.outlier.thres
residuals=rnorm(mean=10,n=1000) 
plot.residuals.multi(residuals=residuals,pred=pred)

#User specification of setting par
residuals=rnorm(mean=0,n=1000) 
plot.residuals.multi(residuals=residuals,pred=pred,resid.outlier.thres=NA,user.par=list(oma=c(0,0,3,0),mar=(c(4,4,3,2)+0.1),mfrow=c(2,4)))

#Don't reset par
residuals1=rnorm(mean=0,n=1000) 
residuals2=rnorm(mean=0,sd=5,n=1000) 
plot.residuals.multi(residuals=residuals1,pred=pred,resid.outlier.thres=NA,plot.type=c("density","qqnorm"),user.par=list(oma=c(0,0,3,0),mar=(c(4,4,3,2)+0.1),mfcol=c(2,2)),reset.par=FALSE)
plot.residuals.multi(residuals=residuals2,pred=pred,resid.outlier.thres=NA,plot.type=c("density","qqnorm"),user.par=NA)

# Demo of current limitations, unable to properly mask cumsum-index plot using resid.outlier.thres
residuals=rnorm(n=1000) 
plot.residuals.multi(residuals=residuals,pred=pred,plot.type=c("index","cumsum-index"),resid.outlier.thres=1.0)
